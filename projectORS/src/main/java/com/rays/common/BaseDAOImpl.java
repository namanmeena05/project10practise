...
public abstract class BaseDAOImpl<T extends BaseDTO> implements BaseDAOInt<T> {
	...
	protected TypedQuery<T> createCriteria(T dto, UserContext userContext) {
		...
		// Specify the root entity (Person) in the query
		Root<T> qRoot = cq.from(getDTOClass());

		// Select all columns from the root entity
		cq.select(qRoot);
		...
		System.out.println(" isgroup skipped......Naman");
		cq.where(whereClause.toArray(new Predicate[whereClause.size()]));

		List<Order> orderBys = getOrderByClause(dto, builder, qRoot);

		System.out.println("after getOrder By clause........Naman");
		cq.orderBy(orderBys.toArray(new Order[orderBys.size()]));
		
		System.out.println("After order by clause.......Naman ");
		TypedQuery<T> query = entityManager.createQuery(cq);
		System.out.println("createCriteria end here---....Naman");
		return query;
	...
	public long add(T dto, UserContext userContext) {
		...
		System.out.println("before calling persist method in base dao......Naman");
...
System.out.println("baseDAO in getOrderByClause.......Naman ");
